...
export async function generateMetadata({ params }) {
  ...
  const pageUrl = `https://himovies-us.netlify.app/tv-series/genre/${genreName}`;
  const imageUrl = 'https://live.staticflickr.com/65535/54798923667_dc8a442f8d_b.jpg';

  return {
    title: `Himovies - ${genreTitle} TV Series`,
    description: `Find and watch the best ${genreTitle} TV series for free on Himovies.`,
    openGraph: {
      title: `Himovies - ${genreTitle} TV Series`,
      description: `Find and watch the best ${genreTitle} TV series for free on Himovies.`,
      url: pageUrl,
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${genreTitle} genre TV series poster`,
        },
      ],
      locale: 'en_US',
      type: 'website',
...
export default async function TvSeriesByGenrePage({ params }) {
  ...
  const initialSeries = await getTvSeriesByGenre(genreId, 1);
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-slate-900 min-h-screen">
      ...
      {initialSeries && initialSeries.length > 0 ? (
        <>
          <MovieList movies={initialSeries} />
